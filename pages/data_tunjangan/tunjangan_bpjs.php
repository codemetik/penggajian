		...
		<div class="card">
			...
			<div class="card-body table-responsive p-0" style="height: 450px;">
				<table class="table table-head-fixed text-nowrap font-10">
					...
					<tbody>
					<?php 
					$sql = mysqli_query($koneksi, "SELECT * FROM tb_tunjangan X INNER JOIN tb_tunjangan_user Y ON y.id_tunjangan = x.id_tunjangan INNER JOIN tb_user z ON z.id_user = y.id_user WHERE y.id_tunjangan = 'TUNJ002'");
...
<form action="pages/proses/proses_tunjangan_bpjs.php" method="POST">
                ...
                $user = mysqli_query($koneksi, "SELECT * FROM tb_user");
  				while ($duser = mysqli_fetch_array($user)) {
  					...
  					if ($dcek['masa_kerja'] >= 12) {
  						...
  						$cektu = mysqli_query($koneksi, "SELECT * FROM tb_tunjangan_user WHERE id_user = '".$dcek['id_user']."' AND id_tunjangan = 'TUNJ002'");
