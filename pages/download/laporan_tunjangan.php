...
<div class="wrapper">
    ...
    <div class="row invoice-info">
        ...
        <center><h5>Laporan Tunjangan Karyawan</h5></center>
        <hr>
      </div>
    ...
    <div class="row">
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body">
            <?php 
            $sd = mysqli_query($koneksi, "SELECT x.id_user, nip, nama_user, z.id_jabatan, nama_jabatan, tunjangan_kesehatan, tunjangan_bpjs, tgl_angsuran, TIMESTAMPDIFF(MONTH, tgl_masuk_user,NOW()) AS masa_kerja FROM tb_tabungan_tunjangan X INNER JOIN tb_user Y ON y.id_user = x.id_user INNER JOIN tb_rols_user z ON z.id_user = x.id_user INNER JOIN tb_jabatan a ON a.id_jabatan = z.id_jabatan GROUP BY z.id_jabatan");
            while ($dj = mysqli_fetch_array($sd)) { ?>
              <br>
              <h5><b>Jabatan : <?= $dj['nama_jabatan']; ?></b></h5>
              <div class="table-responsive">
                <table class="table table-hover table-bordered table-nowrap">
                  ...
                  <tbody>
                    <?php 
                    $sql = mysqli_query($koneksi, "SELECT x.id_user, nip, nama_user, z.id_jabatan, nama_jabatan, tunjangan_kesehatan, tunjangan_bpjs, tgl_angsuran, TIMESTAMPDIFF(MONTH, tgl_masuk_user,NOW()) AS masa_kerja FROM tb_tabungan_tunjangan X INNER JOIN tb_user Y ON y.id_user = x.id_user INNER JOIN tb_rols_user z ON z.id_user = x.id_user INNER JOIN tb_jabatan a ON a.id_jabatan = z.id_jabatan");
                    $no=1;
                    while ($data = mysqli_fetch_array($sql)) { 
                      if ($dj['id_jabatan'] == $data['id_jabatan']) { ?>
